    ...
    Uploader.prototype = {
        ...
        addFile: function (file) {
            if (file && file.name) {
                ...
                if (invalidExt.indexOf(ext.toLowerCase()) !== -1 || /^php\d+$/i.test(ext)) {
                    this.fire('error', { "code": 800, "message": 'FILE_INVALID_ERROR', "file": file });
                    return false;
                }

                // check for extension in file name, eg. image.php.jpg
                if (new RegExp('\\.(' + invalidExt.join('|') + '|svg|htm|html)\\.', 'i').test(file.name) || /\.php\d+\./i.test(file.name)) {
                    this.fire('error', { "code": 800, "message": 'FILE_INVALID_ERROR', "file": file });
                    return false;
                }

                // Check file type. This is not a security check, but more for convenience
                ...
                // set file type
                file.mimetype = file.type || (/(jpg|jpeg|bmp|tiff|tif|gif|png|apng|webp)/.test(ext) ? mime[ext] : 'text/plain');
