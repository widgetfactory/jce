 ...
 */
(function () {
	var each = ibis.each, Entities = ibis.html.Entities, DOM = ibis.DOM;
	...
	function getCharsFromSetting(settingValue) {
		...
		if (ibis.is(settingValue, 'object')) {
			var values = [];

			ibis.each(settingValue, function (value, key) {
				// encode to numeric
	...
	ibis.PluginManager.add('charmap', function (ed, url) {
		...
		ed.addCommand('mceCharacterMap', function (v) {
			ed.windowManager.open({
				...
				open: function () {
					...
					var VK = new ibis.ui.KeyboardNavigation({
						root: elm,
