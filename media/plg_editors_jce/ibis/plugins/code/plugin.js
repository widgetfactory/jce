(function () {
    ...
    const Node$1 = ibis.html.Node;

    /**
     * Create a ibis text node
     * @param {String} value
     * @param {Boolean} raw
     */
    ...
    function init(editor) {
        var state = {
            htmlSchema: new ibis.html.Schema({
                schema: 'mixed',
    ...
    function isInvalidElement(editor, name) {
        ...
        return ibis.inArray(invalid_elements, name) !== -1;
    }
    ...
    ibis.PluginManager.add('code', function (editor, url) {
        ...
        editor.onPreInit.add(function () {
            ...
            editor.parser.addNodeFilter('script,style,link', function (nodes) {
                ...
                while (i--) {
                    ...
                    if (!code_blocks) {
                        ...
                        if (node.firstChild) {
                            value = ibis.trim(node.firstChild.value);
                        }
            ...
            editor.parser.addAttributeFilter('data-mce-code', function (nodes, name) {
                ...
                function isBlockNode(node) {
                    return ibis.inArray(blockElements, node.name) != -1;
                }

                function isInlineTextNode(node) {
                    return ibis.inArray(inlineElements, node.name) != -1;
                }
