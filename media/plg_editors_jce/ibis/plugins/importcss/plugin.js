...
(function () {
  var each = ibis.each,
    DOM = ibis.DOM, PreviewCss = ibis.util.PreviewCss;
  ...
  function isEditorContentCss(url) {
    return /\/(ibis|plugins\/jce)\//.match(url) !== -1 && url.indexOf('content.css') !== -1;
  }
  ...
  ibis.PluginManager.add('importcss', function (ed, url) {
    var self = this;

    // create an event to use externally
    ed.onImportCSS = new ibis.util.Dispatcher();

    ed.onImportCSS.add(function () {
      if (ibis.is(ed.settings.importcss_classes)) {
        return;
      }
    ...
    this.get = function () {
      ...
      if (classes.length) {
        ...
        ed.settings.importcss_classes = ibis.map(classes, function (val) {
