(function () {
   ...
   *
   * License: http://www.ibis.com/license
   * Contributing: http://www.ibis.com/contributing
   */
  ...
  var RangeUtils$1 = ibis.dom.RangeUtils;
  ...
  var getSelectedSubLists = function (editor) {
    ...
    return ibis.grep(editor.selection.getSelectedBlocks(), function (elm) {
      return NodeType.isListNode(elm) && parentList !== elm;
    });
  };

  var findParentListItemsNodes = function (editor, elms) {
    var listItemsElms = ibis.map(elms, function (elm) {
  ...
  var getSelectedListItems = function (editor) {
    ...
    return ibis.grep(findParentListItemsNodes(editor, selectedBlocks), function (block) {
  ...
  var normalizeLists = function (dom, element) {
    ibis.each(ibis.grep(dom.select('ol,ul', element)), function (ul) {
  ...
  var splitList = function (editor, ul, li, newBlock) {
    ...
    var removeAndKeepBookmarks = function (targetNode) {
      ibis.each(bookmarks, function (node) {
  ...
  var BookmarkManager = ibis.dom.BookmarkManager;
  ...
  var setAttribs = function (elm, attrs) {
    ibis.each(attrs, function (value, key) {
  ...
  var updateListAttrs = function (dom, el, detail) {
    ...
    ibis.each(dom.select('li', el), function (li) {
  ...
  var getSelectedTextBlocks = function (editor, rng) {
    ...
    ibis.each(siblings, function (node) {
  ...
  var applyList = function (editor, listName, detail) {
    ...
    ibis.each(getSelectedTextBlocks(editor, rng), function (block, idx) {
  ...
  var removeList = function (editor) {
    ...
    var emptyListItems = ibis.grep(listItems, function (li) {
      return editor.dom.isEmpty(li);
    });

    listItems = ibis.grep(listItems, function (li) {
      return !editor.dom.isEmpty(li);
    });

    ibis.each(emptyListItems, function (li) {
    ...
    ibis.each(listItems, function (li) {
      var node, rootList;
  ...
  var toggleMultipleLists = function (editor, parentList, lists, listName, detail) {
    if (parentList.nodeName === listName && !hasListStyleDetail(detail)) {
      ...
      ibis.each([parentList].concat(lists), function (elm) {
  ...
  var RangeUtils = ibis.dom.RangeUtils, TreeWalker = ibis.dom.TreeWalker, VK$1 = ibis.VK;
  ...
  ibis.PluginManager.add("lists", function (editor) {
    Delete.setup(editor);
    ...
    editor.onNodeChange.add(function (ed, cm, n, collapsed, args) {
      var lists = ibis.grep(args.parents, NodeType.isListNode);

      ibis.each(iconMap, function (listName, btnName) {
