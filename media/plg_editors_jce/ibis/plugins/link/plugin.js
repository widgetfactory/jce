 ...
 */
(function () {
    var DOM = ibis.DOM, Event = ibis.dom.Event, each = ibis.each, extend = ibis.extend;

    // A selection of useful utilities borrowed from https://github.com/ibis/ibis/blob/develop/modules/ibis/src/plugins/link/main/ts/core/Utils.ts
    ...
    var collectNodesInRange = function (rng, predicate) {
        if (rng.collapsed) {
            ...
            var walker = new ibis.dom.TreeWalker(contents.firstChild, contents);
            var elements = [], nodes = [];
    ...
    var getTextNodes = function (elm) {
        ...
        var walker = new ibis.dom.TreeWalker(node, elm);
    ...
    var updateTextContent = function (elm, text) {
        ...
        ibis.each(nodes.slice(1), function (node) {
            node.nodeValue = '';
        });
    };
    ...
    function createLink(ed, data) {
        ...
        args = ibis.extend(args, params.attributes || {});
    ...
    ibis.PluginManager.add('link', function (ed, url) {
        ...
        var urlCtrl, textCtrl, titleCtrl, targetCtrl, stylesListCtrl;

        function createBasicDialog() {
            var cm = ed.controlManager;

            if (cm.get('link_form')) {
                return cm.get('link_form');
            }

            var form = cm.createForm('link_form');
