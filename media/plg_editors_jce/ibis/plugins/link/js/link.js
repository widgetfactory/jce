...
/* global Wf, jQuery, ibisPopup, WfLightboxAdapter */
...
(function ($, ibisPopup) {
    ...
    /**
     * From ibis/plugins/link/plugin.js
     *
    ...
    var LinkDialog = {
        ...
        init: function () {
            ...
            $('#search-options-button').on('click', function (e) {
                ...
                $('#search-options').height($p.parent().height() - $p.outerHeight() - 32).toggle();
            ...
            if (api.isAnchor(anchorElm)) {
                ...
                // check for popups
                var data = WfLightboxAdapter.getLightbox(anchorElm) || {};
        ...
        checkPrefix: function (n) {
            ...
            if (emailRex.test(v) && !/^\s*mailto:/i.test(v)) {
                Wf.Modal.confirm(ibisPopup.getLang('link_dlg.is_email', 'The URL you entered seems to be an email address, do you want to add the required mailto: prefix?'), function (state) {
                    if (state) {
                        $(n).val('mailto:' + v);
                    }
                    self.insertAndClose();
                ...
                Wf.Modal.confirm(ibisPopup.getLang('link_dlg.is_external', 'The URL you entered seems to be an external link, do you want to add the required https:// prefix?'), function (state) {
                    if (state) {
                        $(n).val('https://' + v);
                    }
                    self.insertAndClose();
        ...
        insertLink: function (args) {
            var url = ibisPopup.editor.documentBaseURI.toRelative(args.url);
            $('#href').val(url);
                ...
                if (results && !results.error) {
                    ...
                    if (results.length) {
                        $.each(results, function (i, values) {
                            ...
                            $('<h3 class="uk-margin-top uk-margin-left uk-text-bold">' + values.label + '</h3>').appendTo('#search-result');
                            ...
                            $.each(values.data, function (i, item) {
                                ...
                                $('<dt class="link uk-margin-small" />').text(item.title).on('click', function () {
                                    var url = item.url, text = item.title;
                                ...
                                if (item.anchors) {
                                    $.each(item.anchors, function (i, a) {
                                        $('<dd class="anchor"><i role="presentation" class="uk-icon uk-icon-anchor uk-margin-small-right"></i>#' + a + '</dd>').on('click', function () {
                                            var url = Wf.String.decode(item.url) + '#' + a;
    ...
    ibisPopup.onInit.add(LinkDialog.init, LinkDialog);
