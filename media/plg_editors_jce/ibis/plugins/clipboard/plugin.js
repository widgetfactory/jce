(function () {
    ...
    var DOM = ibis.DOM, each$1 = ibis.each;
    ...
    function openWin(ed, cmd) {
        ...
        msg = msg.replace(/%s/g, ibis.isMac ? 'CMD' : 'CTRL');
        ...
        function createEditor(elm) {
            var pasteEd = new ibis.Editor(elm.id, {
                plugins: '',
                language_load: false,
            ...
            pasteEd.onPreInit.add(function () {
                ...
                pasteEd.onPastePostProcess.add(function (ed, o) {
                    each$1(dom.select('img[data-mce-upload-marker]', o.node), function (img) {
                        dom.setAttrib(img, 'src', ibis.util.Env.transparentSrc);
            ...
            buttons: [
                ...
                },
                {
                    ...
                    onsubmit: function (e) {
                        ...
                        if (cmd == "mcePaste") {
                            var content = ibis.get(inp.id).getContent();
                            // Remove styles
                            ...
                            // trim and assign
                            data.content = ibis.trim(content);
                            // set internal flag
    ...
    ibis.PluginManager.add('clipboard', function (ed, url) {
        ...
        var FakeClipboard = ibis.clipboard.FakeClipboard;
        ...
        each(['mcePasteText', 'mcePaste'], function (cmd) {
            ed.addCommand(cmd, function (ui) {
                ...
                // clipboard.read() unavailable or no clipboard API — open the dialog.
