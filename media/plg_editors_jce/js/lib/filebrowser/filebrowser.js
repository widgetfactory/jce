...
(function ($, Wf, undef) {
    ...
    FileBrowser.prototype = {
        ...
        /**
         * Number of configured directory stores. The store is a keyed object,
         * so count its keys (NB: store.length is undefined).
         */
        getStoreLength: function () {
            return Object.keys(this.getDirectoryStore()).length;
        },
        ...
        _init: function (options) {
            ...
            var grid_size = this._getState('grid-size', 4);

            $('.grid-size', '#browser').on('click', 'button', function (e) {
                e.preventDefault();

                var btn = $(e.target).closest('button');
                var size = $(btn).parent().data('size');
                ...
                // disable minus button if size = 1
                if (size === 1) {
                    $(btn).parent().find('.grid-size-plus').prop('disabled', true);
                } else {
                    $(btn).parent().find('.grid-size-plus').prop('disabled', false);
                }

                if (size == 4) {
                    $(btn).parent().find('.grid-size-minus').prop('disabled', true);
                } else {
        ...
        _setupDir: function () {
            ...
            var src = $(this.element).val() || this.options.source || '';
        ...
        _buildList: function (o) {
            ...
            // hide upload/new-folder only at the multi-store virtual root (no current directory to act in)
            var isRoot = this._isRoot(), storeLen = this.getStoreLength();
        ...
        _resetStatus: function () {
            ...
            if (dir) {
                ...
                // split to array and map to crumb objects, accumulating the path for each segment
                var crumbPath = '';

                var crumbs = path.split('/').map(function (name) {
                    crumbPath = Wf.String.path(crumbPath, name);

                    return {
                        name: name,
                        path: prefix + ':' + crumbPath
                    };
                });

                var store = this.getDirectoryStore(prefix), label = store ? store.label : '';

                // prepend the store label as its own crumb (links to the store root); multi-store only
                if (label && self.getStoreLength() > 1) {
                    crumbs.unshift({
                        name: label,
                        path: prefix + ':'
                    });
                ...
                $.each(crumbs, function (i, crumb) {
                    ...
                    var $item = $('<li title="' + Wf.String.encode(crumb.name) + '"></li>').on('click', 'a', function (e) {
                        var path = $(e.target).data('path');
                        self._changeDir(path);
                    }).append('<a data-path="' + Wf.String.encode(crumb.path) + '">' + Wf.String.encode(crumb.name) + '</a>').insertBefore($count);
        ...
        _isRoot: function () {
            ...
            // remove slashes
            val = this._trimPath(val);

            if (val.indexOf(':') !== -1) {
                var parts = val.split(':');

                if (parts[1] === '') {
                    return this.getStoreLength() === 1;
                }
            }
        ...
        _getItemDetails: function () {
            ...
            if ($(item).data('permissions')) {
                $('.uk-comment-header', info).append('<div class="uk-comment-meta" id="info-permissions">' + ibisPopup.getLang('dlg.permissions', 'Permissions') + ': ' + Wf.String.encode($(item).data('permissions')) + '</div>');
            }
                ...
                self.serializeItemData(item).then(function (data) {
                    ...
                    function callback(data) {
                        ...
                        if (data.width && data.height) {
                            ...
                            if (mime && mime === "image") {
                                $('#info-preview').empty().append('<img src="' + Wf.String.encode(data.preview) + '" alt="" />').removeClass('loading');
                            }
                        }
