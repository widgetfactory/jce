 ...
 *
 * Adapters: WFJoomlaFileSystem (local), plus the s3, azure, webdav and server plugins.
 */
class WFFileSystem extends WFExtension
{
    ...
    /**
     * Get a filesystem adapter instance, falling back to the joomla adapter.
     *
     * Instances are cached per type and config, so repeated calls in a request share one object.
     *
     * @param string $type   The filesystem extension name, eg: joomla, s3
     * @param array  $config Configuration passed to the adapter constructor
     *
     * @return WFFileSystem The adapter, or this base class if the class cannot be found
     */
     ...
     *
     * @return WFFileSystemResult|bool The result object, or true when nothing was done
     */
     ...
     * @param string $conflict How to resolve an existing name, replace or copy
     *
     * @return WFFileSystemResult|bool The result object, or true when nothing was done
     */
    public function copy($src, $dest, $conflict = 'replace')
    {
        return true;
    }
