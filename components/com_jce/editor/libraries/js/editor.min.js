...
(function () {
    ...
    var WfEditor = {
        ...
        createInstance: function (el) {
            ...
            // get toggle option
            var toggle = tinymce.is(s.toggle) ? parseInt(s.toggle, 10) : 0;
        ...
        getActiveTab: function (ed) {
            var activeTabs = DOM.select('ul.nav-tabs > li.active button', ed.getElement().parentNode);

            // no active tabs, default to editor
            if (!activeTabs.length) {
                return ed;
            }

            var ctrl = activeTabs[0].getAttribute('aria-controls') || 'wf-editor-wysiwyg';
        ...
        createToggle: function (el) {
            var self = this,
                s = this.settings,
                toggle = tinymce.is(s.toggle) ? s.toggle : 0;
        ...
        createTabs: function (ed) {
            ...
            // Editor tab
            var editorTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a role="presentation"><button type="button" aria-controls="wf-editor-wysiwyg" aria-expaned="false" role="tab">' + ed.getLang('tab.wysiwyg', 'Editor') + '</button></a>');

            // Source tab
            if (tinymce.inArray(plugins, 'source') !== -1) {
                var sourceTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a role="presentation"><button type="button" aria-controls="wf-editor-source" aria-expaned="false" role="tab">' + ed.getLang('tab.code', 'Code') + '</button></a>');
            ...
            if (tinymce.inArray(plugins, 'preview') !== -1) {
                var previewTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a role="presentation"><button type="button" aria-controls="wf-editor-preview" aria-expaned="false" role="tab">' + ed.getLang('tab.preview', 'Preview') + '</button></a>');
        ...
        getContent: function (id) {
            ...
            var elm = document.getElementById(id), state = this.getEditorState(elm);
