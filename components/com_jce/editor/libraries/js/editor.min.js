    ...
    var WFEditor = {
        ...
        load: function () {
            ...
            Event.bind(window, 'load', function () {
                ...
                if (window.jQuery) {
                    ...
                    // patch in "stop" event for subform sortable to trigger a "subform-row-sort" event
                    jQuery(document).ready(function ($) {
                        $sortable = $('div.subform-repeatable').find('.ui-sortable');

                        if ($sortable.length && !$sortable.sortable('option', 'stop')) {
                            $sortable.sortable('option', {
                                'stop': function (event, ui) {
                                    $(document).trigger('subform-row-sort', ui.item[0]);
                                }
                            });
                        }
                    });
                    ...
                    jQuery(document).on('subform-row-sort editor-reinit', function (e, row) {
                        var elements = DOM.select('.wf-editor', row);
        ...
        createInstance: function (el) {
            ...
            if (s.use_state_cookies !== false) {
                state = tinymce.util.Cookie.get('wf_editor_state_' + el.id);
            }
        ...
        _toggleEditor: function (el) {
            ...
            if (!ed) {
                ...
                if (this.settings.use_state_cookies !== false) {
                    tinymce.util.Cookie.set('wf_editor_state_' + el.id, 1);
                }
                ...
                if (DOM.hasClass(el, 'wf-no-editor')) {
                    ...
                    // store state
                    if (this.settings.use_state_cookies !== false) {
                        tinymce.util.Cookie.set('wf_editor_state_' + el.id, 0);
                    }
        ...
        _createTabs: function (ed) {
            ...
            // Editor tab
            var editorTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a href="#wf-editor-wysiwyg" class="nav-link" role="tab">' + ed.getLang('tab.wysiwyg', 'Editor') + '</a>');

            // Source tab
            if (tinymce.inArray(plugins, 'source') !== -1) {
                var sourceTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a href="#wf-editor-source" class="nav-link" role="tab">' + ed.getLang('tab.code', 'Code') + '</a>');
            ...
            if (tinymce.inArray(plugins, 'preview') !== -1) {
                var previewTab = DOM.add(ul, 'li', { 'class': 'nav-item', 'role': 'presentation' }, '<a href="#wf-editor-preview" class="nav-link" role="tab">' + ed.getLang('tab.preview', 'Preview') + '</a>');
