    ...
    var WFEditor = {
        ...
        load: function () {
            ...
            tinyMCE.onAddEditor.add(function (mgr, ed) {
                ...
                if (s.compress.css) {
                    ed.onPreInit.add(function () {
                        ed.dom.loadCSS(s.site_url + 'index.php?option=com_jce&view=editor&task=pack&type=css&layout=content&context=' + s.context + '&' + s.token + '=1');
                    });
                }
                ...
                var elm = ed.getElement();

                var elmWidth   = s.width    || elm.style.width || '';
                var elmHeight  = s.height   || elm.style.height || ''; 
                ...
                ed.onPostRender.add(function () {
                    ...
                    if (elmHeight) {
                        DOM.setStyle(elm, 'height', elmHeight);
                    }

                    // Resize iframe and container
                    if (elmWidth) {
                        DOM.setStyle(e.parentNode, 'max-width', elmWidth);
                        DOM.setStyle(elm, 'max-width', elmWidth);
                    }
                });
        ...
        _createToggle: function (el) {
            ...
            if (DOM.select('a.wf-editor-toggle', header).length === 0) {
                // create toggle button
                btn = DOM.add(header, 'a', {
                    'href': '#',
        ...
        _toggleTab: function (ed, tab) {
            ...
            if (!isHidden(ed.id + '_parent') && !DOM.hasClass(el.parentNode, 'mce-fullscreen')) {
        ...
        _createTabs: function (ed) {
            ...
            if (tinymce.inArray(plugins, 'preview') !== -1) {
                var previewTab = DOM.add(ul, 'li', {}, '<a href="#wf-editor-preview">' + ed.getLang('tab.preview', 'Preview') + '</a>');
            ...
            if (tinymce.inArray(plugins, 'source') !== -1) {
                var sourceTab = DOM.add(ul, 'li', {}, '<a href="#wf-editor-source">' + ed.getLang('tab.code', 'Code') + '</a>');
            ...
            // Editor tab
            var editorTab = DOM.add(ul, 'li', {}, '<a href="#wf-editor-wysiwyg">' + ed.getLang('tab.wysiwyg', 'Editor') + '</a>');
        ...
        /**
         * Get the editor selection content
         * @param {String} id The editor id
         */
