...
class Browser
{
    ...
    protected function getFormAction()
    {
        ...
        $args = array(
            'plugin' => $this->getContainer()->getName(),
            'context' => $context,
        );
    ...
    public function setFileTypes($list = 'jpg,jpeg,png,gif')
    {
        ...
        $upload = $this->config->getArray('upload', array());
    ...
    private function getDirectoryStore()
    {
        ...
        if (empty($dir)) {
            ...
            $event = new Event('onWfFileSystemGetRootDir', array(
                'path' => $path,
                'label' => $label,
                'subject' => $this
            ));

            $dispatcher->dispatch('onWfFileSystemGetRootDir', $event);

            $path = trim($event->getArgument('path', $path));
    ...
    public function getDirectoryStoreFromPath($path, $withKey = false)
    {
        ...
        if (empty($prefix)) {
            ...
            foreach ($store as $key => $value) {
                ...
                if (StringHelper::safe_strpos($path, $value['path']) === 0) {
    ...
    private function getPathVariables()
    {
        ...
        if (!isset($variables)) {
            ...
            $event = new Event('onWfFileSystemBeforeGetPathVariables', [
                'variables' => $variables,
                'subject'   => $this
            ]);

            $app->getDispatcher()->dispatch('onWfFileSystemBeforeGetPathVariables', $event);
        ...
        $event = new Event('onWfFileSystemGetPathVariables', [
            'variables' => $variables,
            'subject'   => $this
        ]);

        $app->getDispatcher()->dispatch('onWfFileSystemGetPathVariables', $event);
    ...
    private function resolveFilterPath($store, $filter)
    {
        ...
        // make the source relative to the store path, eg: stories => images/stories
        $filterPath = Utility::makePath($store['path'], $filter);
    ...
    public function searchItems($path, $limit = 25, $start = 0, $query = '', $sort = '')
    {
        ...
        foreach ($terms as $term) {
            if (
                ...
                // It's an extension
                $extensions[] = Utility::makeSafe($term);
            } elseif ($term !== '') {
        ...
        if ($query[0] === '.') {
            ...
            $extension = Utility::makeSafe($query);
        ...
        foreach ($storeArray as $storeItem) {
            ...
            foreach ($items as $item) {
                ...
                if (StringHelper::safe_strpos($item['id'], $storeItem['path']) === 0) {
                    $item['id'] = StringHelper::safe_substr($item['id'], StringHelper::safe_strlen($storeItem['path']));
    ...
    public function getItems($source, $limit = 25, $start = 0, $filter = '', $sort = '')
    {
        ...
        if (empty($path) || empty($store)) {
            ...
            if (!$prefix) {
                ...
                if (StringHelper::safe_strpos($path, $store['path']) === 0) {
                    $path = StringHelper::safe_substr($path, StringHelper::safe_strlen($store['path']));
                    // trim
                    $path = trim($path, '/');
                }
            } else {
        ...
        if ($filter) {
            if ($filter[0] == '.') {
                ...
                $name = '^(?i).*' . Utility::makeSafe($filter) . '.*';
            }
        }
        ...
        if (count($items)) {
            ...
            foreach ($items as $item) {
                ...
                // remove the $store['path'] value from the beginning of the id, must be multibyte safe
                if (StringHelper::safe_strpos($item['id'], $store['path']) === 0) {
                    $item['id'] = StringHelper::safe_substr($item['id'], StringHelper::safe_strlen($store['path']));
                }
                ...
                // create path
                $item['path'] = Utility::makePath($store['path'], $item['id']);
    ...
    public function getTreeItem($path = "")
    {
        ...
        if (empty($path)) {
            ...
            array_walk($folders, function (&$item) use ($store, $path) {
                ...
                $item['id']     = $store['prefix'] . ':' . $item['id'];
                $item['path']   = Utility::makePath($path, $item['name']);
                $item['class']  = 'folder';
            });
        }
    ...
    public function upload()
    {
        ...
        if ($result instanceof FilesystemResult) {
            if ($result->state === true) {
                ...
                if (empty($result->url)) {
                    /*$relative = Utility::makePath($dir, $name);
                    $result->url = Utility::makePath($filesystem->getBaseURL(), $relative);*/

                    $result->url = Utility::makePath($dir, $name);
                }
            ...
            if ($result instanceof FilesystemResult) {
                if (!$result->state) {
                    if ($result->message) {
                        ...
                        $this->setResult(Text::sprintf('WF_MANAGER_DELETE_' . strtoupper($result->type) . '_ERROR', Utility::mb_basename($item)), 'error');
                    }
                } else {
    ...
    public function renameItem()
    {
        ...
        if ($result instanceof FilesystemResult) {
            if (!$result->state) {
                $this->setResult(Text::sprintf('WF_MANAGER_RENAME_' . strtoupper($result->type) . '_ERROR', Utility::mb_basename($source)), 'error');
                if ($result->message) {
                    $this->setResult($result->message, 'error');
                }
            } else {
            ...
            if ($result instanceof FilesystemResult) {
                if (!$result->state) {
                    if ($result->message) {
                        ...
                        $this->setResult(Text::sprintf('WF_MANAGER_COPY_' . strtoupper($result->type) . '_ERROR', Utility::mb_basename($item)), 'error');
                    }
                } else {
                    $data = array(
            ...
            if ($result instanceof FilesystemResult) {
                if (!$result->state) {
                    if ($result->message) {
                        ...
                        $this->setResult(Text::sprintf('WF_MANAGER_MOVE_' . strtoupper($result->type) . '_ERROR', Utility::mb_basename($item)), 'error');
                    }
                } else {
                    $data = array(
                        'name' => $filesystem->toRelative($result->path),
    ...
    private function getUploadValue()
    {
        ...
        $upload = Utility::convertSize($upload);
        $post = Utility::convertSize($post);
