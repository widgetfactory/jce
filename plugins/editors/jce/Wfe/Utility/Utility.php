...
abstract class Utility
{
    ...
    public static function uriToAbsolutePath($url)
    {
        ...
        $path = StringHelper::safe_substr($url, 0, StringHelper::safe_strlen($root));

        if ($path === $root) {
            $relativePath = StringHelper::safe_substr($url, StringHelper::safe_strlen($root));
    ...
    public static function checkName($name)
    {
        ...
        // covers the encoded forms (eg. %22, %3C). Note: "&", "+", "$" and "'" are allowed - they
        // URL-encode cleanly and are HTML-encoded on output (eg. "'" becomes &#39;), so a legitimate
        // filename such as O'Brien.pdf remains listable. Only "<", ">" and '"' are blocked as a
        // server-side backstop - these never occur in valid filenames and are invalid on Windows.
        if (preg_match('#[<>"]#', $name)) {
            throw new \InvalidArgumentException('Invalid name');
        }
    ...
    {
        if ($size < 1024) {
            return $size . ' ' . Text::_('WF_LABEL_BYTES');
        }

        if ($size < 1048576) { // 1024 * 1024
            return sprintf('%.2f', $size / 1024) . ' ' . Text::_('WF_LABEL_KB');
        }

        return sprintf('%.2f', $size / 1048576) . ' ' . Text::_('WF_LABEL_MB');
    }
     ...
     * (Normalizer) is unavailable. Covers the homoglyph vector that matters for extensions -
     * fullwidth letters/digits and the fullwidth full stop U+FF0E - without touching genuine scripts.
     *
    ...
    public static function validateFileName($name, $allowedExtensions = [])
    {
        ...
        // fold unicode compatibility characters to their ascii form for the checks below, so
        // homoglyphs (eg. fullwidth "ｐｈｐ", or U+FF0E used as a dot) cannot slip past.
        ...
        // normalise each part before comparison. Windows strips trailing dots and spaces
        // from file names, NTFS treats ':' as an alternate data stream separator, and older
        // IIS truncates at ';' — so "php ", "php:" and "php;" must all match "php" below.
