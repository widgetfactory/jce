...
class Plugin extends \Wfe\Editor\Plugin\Manager\BaseManager
{
    ...
    public function __construct($config = array())
    {
        ...
        $config['layout'] = 'browser';
        $config['can_edit_images'] = 1;
        $config['show_view_mode'] = 1;
        ...
        if (!$standalone || $this->isMediaField()) {
            ...
            $browser->setFileTypes(implode(',', array_values(array_unique($selected))));
        }
    ...
    private function getMediaFolder()
    {
        ...
        if ($folder) {
            ...
            foreach ($parts as $key => $part) {
                $parts[$key] = Utility::makeSafe($part);
            }
    ...
    public function display()
    {
        ...
        if ($document->getConfig('standalone') == 1) {
            ...
            if ($this->isMediaField()) {
                $document->setConfig('mediafield', 1);
            }
